...
export default function LearnLevelPicker() {
  ...
  const [mode, setMode] = useState<'browse' | 'my_words'>('browse');
  ...
  const loadForLevel = useCallback(async () => {
    ...
    try {
      const token = await getToken();
      if (mode === 'browse') {
        const data = await apiFetch(`/vocab/topics?level=${level}`, token);
        setTopics(data.results);
      } else {
        const data = await apiFetch(`/vocab?level=${level}&learnt=true`, token);
        setMyWords(data.results);
      }
    } catch {
      if (mode === 'browse') setTopics([]);
      else setMyWords([]);
    } finally {
    ...
    <View style={styles.container}>
      <View style={styles.searchBar}>
        ...
        <TextInput
          ...
          placeholder="Search vocab (kanji, reading, or meaning)"
      ...
      {results === null ? (
        <>
          ...
          <View style={styles.toggleRow}>
            ...
            <Pressable
              style={[styles.toggleButton, mode === 'my_words' && styles.toggleButtonActive]}
              onPress={() => setMode('my_words')}>
              <Text style={[styles.toggleText, mode === 'my_words' && styles.toggleTextActive]}>
                My Words
              </Text>
            </Pressable>
          </View>
          ...
          {!loadingLevel && mode === 'browse' && (
            <FlatList
              data={topics}
              keyExtractor={(item) => item.topic}
              contentContainerStyle={styles.list}
              ListEmptyComponent={<Text style={styles.empty}>No topics seeded for this level yet.</Text>}
              renderItem={({ item }) => (
                <Pressable style={styles.row} onPress={() => router.push(`/learn/vocab/${level}/${item.topic}`)}>
                  <Text style={styles.rowTitle}>{formatTopic(item.topic)}</Text>
            ...
            <FlatList
              ...
              ListEmptyComponent={
                <Text style={styles.empty}>
                  Nothing learnt at this level yet -- mark a word as Learnt to see it here.
                </Text>
              }
              renderItem={({ item }) => (
        ...
        <View style={styles.resultsWrap}>
          ...
          <FlatList
            ...
            renderItem={({ item }) => (
              <Pressable style={styles.resultRow} onPress={() => openWord(item)}>
                {item.kanji && <Text style={styles.resultFurigana}>{item.reading}</Text>}
                <Text style={styles.resultTitle}>{item.kanji ?? item.reading}</Text>
